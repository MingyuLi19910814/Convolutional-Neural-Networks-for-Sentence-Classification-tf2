...
def create_text_cnn_pretrained(sequence_length,
                               num_classes,
                               embedding_size,
                               filter_sizes,
                               num_filters,
                               dropout_keep_prob,
                               l2_reg_lambda=0.0):
    input_x = tf.keras.layers.Input(shape=(sequence_length, 300), dtype=tf.float32)  # (batch_size, sequence_length)
    embedded_x = tf.expand_dims(input_x, -1)
    pooled_outputs = []
    for i, filter_size in enumerate(filter_sizes):
        x = tf.keras.layers.Conv2D(filters=num_filters,
                                   kernel_size=(filter_size, embedding_size),
